...
# Constants
readonly REPO_URL="https://github.com/herschel21/nvim-configuration/archive/refs/heads/minimal-no-shit.zip"
readonly TEMP_DIR="/tmp/nvim_install_$(date +%s)"
readonly DEST_DIR="$HOME/.config"
readonly ZIP_FILE="$TEMP_DIR/minimal-no-shit.zip"
...
install_config() {
    ...
    cp -r "$TEMP_DIR/nvim-configuration-minimal-no-shit/nvim/" "$DEST_DIR/" || error_exit "Failed to copy configuration"
}
